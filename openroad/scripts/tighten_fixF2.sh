#!/bin/bash
# Fix C+D+E+F2 RTL (/rtl/rtl_fix5), nickname riscv_fixF2: 1.00 (decisive), then 1.05, 0.95. Gated on sim pass.
OUT=/rtl/fix_logs; LOG=$OUT/driver_fixF2.log; CSV=$OUT/tighten_fixF2.csv
...
echo "FIXF2_SWEEP_DONE $(date +%H:%M:%S)" >> $LOG
