...
LOG=$OUT/driver_fix3.log
CSV=$OUT/tighten_fix3.csv
...
W=0
until grep -q '^FIX2_SWEEP_DONE' $OUT/driver_fix2.log 2>/dev/null; do sleep 15; W=$((W+15)); [ $W -ge 7200 ] && { echo "[$(date +%H:%M:%S)] fix2-sweep wait TIMEOUT" >> $LOG; exit 1; }; done
...
for P in 1.25 1.20 1.15 1.10; do
  TAG=fix3_p$P
...
echo "FIX3_SWEEP_DONE $(date +%H:%M:%S)" >> $LOG
