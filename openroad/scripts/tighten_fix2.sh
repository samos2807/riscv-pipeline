#!/bin/bash
# Re-tightening sweep on the Fix C+D RTL (/rtl/rtl_fix2), nickname riscv_fix2.
# Gated on: sim pass marker AND completion of the fix1 flow (shared CPU/RAM).
OUT=/rtl/fix_logs; mkdir -p $OUT
LOG=$OUT/driver_fix2.log
CSV=$OUT/tighten_fix2.csv
GATE=/rtl/sim_work/fix2/SIM_PASS
W=0
until [ -f "$GATE" ]; do sleep 3; W=$((W+3)); [ $W -ge 300 ] && { echo "[$(date +%H:%M:%S)] GATE TIMEOUT - sim not passed, NOT running" >> $LOG; exit 1; }; done
echo "[$(date +%H:%M:%S)] sim gate open; waiting for fix1 flow to finish" >> $LOG
W=0
until grep -q '^RESULT fix1' $OUT/driver.log 2>/dev/null; do sleep 15; W=$((W+15)); [ $W -ge 2400 ] && { echo "[$(date +%H:%M:%S)] fix1 wait TIMEOUT" >> $LOG; exit 1; }; done
echo "[$(date +%H:%M:%S)] fix1 done; preparing riscv_fix2 design" >> $LOG

D=/rtl/orfs/designs/nangate45/riscv_fix2; mkdir -p $D
sed -e 's|/rtl/rtl_pipelined/|/rtl/rtl_fix2/|g' \
    -e 's|riscv_pipe/constraint.sdc|riscv_fix2/constraint.sdc|' \
    -e 's|^export DESIGN_NICKNAME = .*|export DESIGN_NICKNAME = riscv_fix2|' \
...
RPT=reports/nangate45/riscv_fix2/base/6_finish.rpt
RLOG=logs/nangate45/riscv_fix2/base/6_report.log
...
for P in 1.30 1.25 1.20 1.15; do
  TAG=fix2_p$P
...
echo "FIX2_SWEEP_DONE $(date +%H:%M:%S)" >> $LOG
